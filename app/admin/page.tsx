...
type Station = { id: string; code: string; name: string; enabled: boolean; volunteers: StationVolunteer[] };
...
export default function AdminPage() {
  ...
  const [newStation, setNewStation] = useState({ code: "", name: "" });
  ...
  const load = useCallback(async () => {
    ...
    setMe(session.user);
    const [eventData, userData, overviewData, syncData, stationData] = await Promise.all([api("/api/admin/events"), api("/api/admin/users"), api("/api/admin/overview"), api("/api/admin/participants/sync"), api("/api/admin/checkin-stations")]);
  ...
  async function createStation() {
    try {
      ...
      setNewStation({ code: "", name: "" }); setMessage("Check-in counter created"); await load();
    } catch (error) { setMessage((error as Error).message); }
  }
