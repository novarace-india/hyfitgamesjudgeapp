...
export default function AdminPage() {
  ...
  if (!me) return <main className="ops-login admin-login"><div className="ops-login-card"><Image src="/branding/hyfit-games-2026-white.svg" width={90} height={90} unoptimized alt="HYFIT Games"/><div className="ops-kicker">EVENT CONTROL</div><h1>Admin sign in</h1><input placeholder="Staff ID" value={login.staffId} onChange={(e) => setLogin({ ...login, staffId: e.target.value })}/><input type="password" inputMode="numeric" placeholder="PIN" value={login.pin} onChange={(e) => setLogin({ ...login, pin: e.target.value })}/><button onClick={signIn}>Open Control Center</button>{message && <p className="ops-error">{message}</p>}</div></main>;

  const active = events.find((event) => event.is_active);
  return <main className="ops-shell admin-shell"><aside className="ops-side"><div className="ops-brand"><Image src="/branding/hyfit-games-2026-white.svg" width={50} height={50} unoptimized alt="HYFIT"/><b>HYFIT CONTROL</b></div>{["overview","events","team","rules","integration","exceptions","audit","system"].map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item.replace(/^\w/, (c) => c.toUpperCase())}</button>)}<div className="ops-user"><b>{me.name}</b><small>{me.staffId} · {me.role}</small></div></aside><section className="ops-content"><header><div><small>ACTIVE EVENT</small><h1>{active?.name ?? "No active event"}</h1><p>{active?.venue}</p></div><span className="ops-live">● {overview?.status ?? "offline"}</span></header>{message && <div className="ops-message">{message}<button onClick={() => setMessage("")}>×</button></div>}
